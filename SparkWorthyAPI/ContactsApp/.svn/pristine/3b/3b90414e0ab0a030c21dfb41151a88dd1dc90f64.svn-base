...
namespace ContactAppAPI.Controllers
{
    ...
    public class LoginController : ApiController
    {
                ...
                if (dtGetUsers.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtGetUsers.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(Convert.ToString(dr[15])))
                            userTypes.ProfilePicture = ConfigurationManager.AppSettings["UserProfile"].ToString().Trim() + Convert.ToString(dr[15]);
        ...
        #region "Update Users Profile Picture"
        ...
        public HttpResponseMessage UpdateUserProfilePicture(int UserId, string uploadDP)
        {
            LoginModels loginModels = new LoginModels();
            loginModels.UserId = UserId;
            loginModels.ProfilePicture = uploadDP;
            string Status = string.Empty;
            if (!string.IsNullOrEmpty(Convert.ToString(loginModels.UserId)))
                Status = loginModels.UpdateProfilePicture();
            return Request.CreateResponse(HttpStatusCode.OK, Status);
        }
